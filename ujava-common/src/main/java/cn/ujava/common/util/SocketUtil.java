...
import cn.ujava.common.command.ExecuteCommand;
    ...
    public static long findTcpListenProcess(int port) {
        try {
            if (OSUtil.isWindows()) {
                ...
                List<String> lines = ExecuteCommand.runNative(command);
            ...
            if (OSUtil.isLinux() || OSUtil.isMac()) {
                String pid = ExecuteCommand.getFirstAnswer("lsof -t -s TCP:LISTEN -i TCP:" + port);
                if (!pid.trim().isEmpty()) {
